...
let emailAddress = "rajan@inklik.com";

// Cloudflare Turnstile — server-side verification
const TURNSTILE_SECRET_KEY = 'xxxxxxxx';
...
function doPost( e ) {
	...
	if ( turnstileToken && !verifyTurnstile( turnstileToken ) ) {
		return HtmlService.createHtmlOutput( "Turnstile verification failed" );
	}

	// Remove turnstile token from data — don't save it to sheet
	delete params['cf-turnstile-response'];
...
/**
 * verifyTurnstile
 * Calls Cloudflare siteverify API to validate the Turnstile token
...
function verifyTurnstile( token ) {
	try {
		...
		// If verification call fails, allow submission (fail-open) so you don't lose leads
		return true;
	}
}
...
const sendNotification = ( data, url ) => {
	...
	for ( let key in data ) {
		if ( ['form_name', 'e_gs_SheetName', 'e_gs_order', 'e_gs_exclude', 'cf-turnstile-response'].includes(key) ) continue;
		body += key + ": " + data[key] + "\n";
	}
	body += "\nGoogle Sheet: " + url;
	...
	MailApp.sendEmail(
		emailAddress,
		"New Lead - " + (data['form_name'] || 'Contact Form'),
		body,
		{
			name: 'InklikDevs Lead Notification'
		}
	);
};


// Test email authorization — Run this manually once, authorize Gmail permissions
function testEmail() {
  MailApp.sendEmail("rajan@inklik.com", "Test", "Test email from Apps Script");
}
